...
import com.jsw.jrb.core.listener.ExcelDictDTOListener;
import com.jsw.jrb.core.pojo.dto.ExcelDictDTO;
import com.jsw.jrb.core.pojo.entity.Dict;
import com.jsw.jrb.core.mapper.DictMapper;
...
public class DictServiceImpl extends ServiceImpl<DictMapper, Dict> implements DictService {
    ...
    public List<Dict> listByParentId(Long parentId) {
        ...
        try {
            dictList = (List<Dict>)redisTemplate.opsForValue().get("jrb:core:dictList:" + parentId);
            if(dictList != null){
                log.info("从redis中取值");
                return dictList;
            }
        ...
        //将数据存入redis
        try {
            redisTemplate.opsForValue().set("jrb:core:dictList:" + parentId, dictList);
            log.info("数据存入redis");
        } catch (Exception e) {
            log.error("redis服务器异常：" + ExceptionUtils.getStackTrace(e));//此处不抛出异常，继续执行后面的代码
        }
